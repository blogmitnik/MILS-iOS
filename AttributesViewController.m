...
- (id)initWithNibName:(NSString *)nibName bundle:(NSBundle *)nibBundle
{
    ...
    if (self) {
        self.title = NSLocalizedString(@"AttributeTitleKey", @"");
        self.navigationItem.title = NSLocalizedString(@"DialogPlayerName", @"");
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row % 2 == 0){  
        ...
        cell.textLabel.text = NSLocalizedString(@"NoGroupKey", @"");
        cell.detailTextLabel.text = NSLocalizedString(@"TapToFindOneKey", @"");
    }
	return cell;
}

-(NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section{
    if(section==0)return  NSLocalizedString(@"GroupTitleKey", @"");
        else return  NSLocalizedString(@"StatsTitleKey", @"");
}
