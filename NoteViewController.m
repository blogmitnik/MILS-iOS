...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        self.title = NSLocalizedString(@"NoteTitleKey", @"");
        ...
        startWithView = 0;
        actionSheet = [[UIActionSheet alloc]initWithTitle:NSLocalizedString(@"SharingTitleKey", @"") delegate:self cancelButtonTitle:NSLocalizedString(@"CancelTitleKey", @"") destructiveButtonTitle:nil otherButtonTitles:NSLocalizedString(@"ListOnlyTitleKey", @""),NSLocalizedString(@"MapOnlyTitleKey", @""),NSLocalizedString(@"BothTitleKey", @""),NSLocalizedString(@"DontShareTitleKey", @""), nil];
    }
    return self;
}
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    if(!self.note.showOnMap && !self.note.showOnList){
        self.sharingLabel.text = NSLocalizedString(@"NoneTitleKey", @"");
    }
    else if(self.note.showOnMap && !self.note.showOnList){
        self.sharingLabel.text = NSLocalizedString(@"MapTitleKey", @"");

    }
    else if(!self.note.showOnMap && self.note.showOnList){
        self.sharingLabel.text = NSLocalizedString(@"ListTitleKey", @"");

    }
    else if(self.note.showOnMap && self.note.showOnList){
        
    }        self.sharingLabel.text = NSLocalizedString(@"ListAndMapTitleKey", @"");
...
- (void)viewDidLoad
{
	...
	[[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"DoneTitleKey", @"")
    ...
    UIBarButtonItem *previewButton = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"PreviewTitleKey", @"") style:UIBarButtonItemStylePlain target:self action:@selector(previewButtonTouchAction)];      
...
-(void)viewWillDisappear:(BOOL)animated{
    self.navigationController.navigationItem.title = NSLocalizedString(@"NoteTitleKey", @"");
...
-(void)actionSheet:(UIActionSheet *)actionSheet willDismissWithButtonIndex:(NSInteger)buttonIndex{
    switch (buttonIndex) {
        case 0:
            ...
            self.sharingLabel.text = NSLocalizedString(@"ListOnlyTitleKey", @"");
            break;
        case 1:
            self.note.showOnList = NO;
            self.note.showOnMap = YES;
            self.sharingLabel.text = NSLocalizedString(@"MapOnlyTitleKey", @"");
            break;
        case 2:
            self.note.showOnList = YES;
            self.note.showOnMap = YES;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if([self.note.contents count] == 0){
        cell.textLabel.text = NSLocalizedString(@"NoContentAddedTitleKey", @"");
        cell.detailTextLabel.text = NSLocalizedString(@"AddSomeContentsTitleKey", @"");
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    timeObserver = [[self.soundPlayer addPeriodicTimeObserverForInterval:time queue:NULL usingBlock:^(CMTime time){ 
        ...
        MILSMoviePlayerViewController *mMoviePlayer = [[MILSMoviePlayerViewController alloc] initWithContentURL:[NSURL URLWithString:media.url]];
