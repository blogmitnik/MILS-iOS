	...
	if (silenceNextServerUpdateCount < 1) {		
		...
		for (Item *item in newInventory) {	
			...
			for (Item *existingItem in self.inventory) {
                ...
                if ((existingItem.itemId == item.itemId) && (existingItem.qty < item.qty)){
                   if([topViewController respondsToSelector:@selector(updateQuantityDisplay)])
                    ...
                    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"ItemReceivedTitleKey", @""),@"title",[NSString stringWithFormat:@"%d %@ %@",item.qty - existingItem.qty,item.name, NSLocalizedString(@"AddedToInventoryTitleKey", @"")],@"prompt", nil];
			...
			if (match == NO) {
                ...
                NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"ItemReceivedTitleKey", @""),@"title",[NSString stringWithFormat:@"%d %@ %@",item.qty,item.name, NSLocalizedString(@"AddedToInventoryTitleKey", @"")],@"prompt", nil];
		...
		if (newItems > 0) {
			...
			if (![AppModel sharedAppModel].hasSeenInventoryTabTutorial){
				[appDelegate.tutorialViewController showTutorialPopupPointingToTabForViewController:self.navigationController  
																							  ...
																							  title:NSLocalizedString(@"NewItemTitleKey", @"")  
																							message:NSLocalizedString(@"NewItemMessageTitleKey", @"")];						
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    lblTemp3.text = [NSString stringWithFormat:@"%@: %d, %@: %d",NSLocalizedString(@"QuantityTitleKey", @""),item.qty,NSLocalizedString(@"WeightTitleKey", @""),item.weight];
    else if(item.weight > 1)
        lblTemp3.text = [NSString stringWithFormat:@"%@: %d",NSLocalizedString(@"WeightTitleKey", @""),item.weight];
    else if(item.qty > 1)
        lblTemp3.text = [NSString stringWithFormat:@"%@: %d",NSLocalizedString(@"QuantityTitleKey", @""),item.qty];
    else
        lblTemp3.text = nil;
        iconView.hidden = NO;
    Media *media;
