...
-(void)loadUserDefaults {
    ...
    if ([[currServ absoluteString] isEqual:@"http://mils.herokuapp.com/server1"] || 
        [[currServ absoluteString]  isEqual:@"http://mils.herokuapp.com/server1/"] || 
        [[currServ absoluteString]  isEqual:@"http://mils.herokuapp.com/stagingserver1"] ||
        [[currServ absoluteString]  isEqual:@"http://mils.herokuapp.com/stagingserver1/"]) {
        
        NSLog(@"AppModel: SERVER NEEDS TO BE CHANGED");
        
        NSString *updatedURL = @"http://mils.herokuapp.com";
...
-(void)removeItemFromInventory:(Item*)item qtyToRemove:(int)qty {
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"ItemLostTitleKey", @""),@"title",[NSString stringWithFormat:@"%d %@ %@",qty,item.name, NSLocalizedString(@"RemovedFromInventoryTitleKey", @"")],@"prompt", nil];
