...
-(void)refreshViewFromModel {
    ...
    MILSAppDelegate* appDelegate = (MILSAppDelegate *)[[UIApplication sharedApplication] delegate];
	progressLabel.text = [NSString stringWithFormat:@"%d %@ %d %@", [AppModel sharedAppModel].currentGame.completedQuests, NSLocalizedString(@"OfQuestTitleKey", @""), [AppModel sharedAppModel].currentGame.totalQuests, NSLocalizedString(@"QuestsCountCompleteTitleKey", @"")];
	...
	if (silenceNextServerUpdateCount < 1) {
		...
		for (Quest *quest in newActiveQuestsArray) {		
			...
			if (match == NO) {
                ...
                activeSort++;
                NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"NewQuestAvailableTitleKey", @""),@"title",quest.name,@"prompt", nil];
        ...
        for (Quest *quest in newCompletedQuestsArray) {		
			...
			if (match == NO) {
                ...
                NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:NSLocalizedString(@"QuestCompletedTitleKey", @""),@"title",quest.name,@"prompt", nil];
		...
		if (newItems > 0) {
			...
			//Put up the tutorial tab
				...
				[appDelegate.tutorialViewController showTutorialPopupPointingToTabForViewController:self.navigationController 
																							  ...
																							  title:NSLocalizedString(@"NewQuestTitleKey", @"") 
																							message:NSLocalizedString(@"NewQuestMessageTitleKey", @"")];						
